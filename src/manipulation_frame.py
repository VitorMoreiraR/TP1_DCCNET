...
def convert_response_to_dictionary(response):
    ...
    frame_without_checksum = SYNC_BYTES + SYNC_BYTES + bytes.fromhex('0000') + length + id + flag
    if data: frame_without_checksum = frame_without_checksum + data
...
def create_frame_confirmation(id):
    ...
    frame = header + bytes.fromhex('0000') + bytes.fromhex('0000') + identifier + FLAG_CONFIRMATION
    checksum = internet_checksum(frame)
    frame = header + checksum + bytes.fromhex('0000') + identifier + FLAG_CONFIRMATION
    
    print(f'SEND -> header: {header} - identifier: {id} - checksum: {checksum.hex()} - flag: {FLAG_CONFIRMATION.hex()}\n')
...
def create_frame_md5(md5_hash, id):
    ...
    print(f'SEND -> header: {header.hex()} - checksum: {checksum.hex()} - length: {int.from_bytes(length)} - identifier: {id} - flag: {flag.hex()} - payload: {payload}\n')
